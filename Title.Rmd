...
```{r result, echo=TRUE, warning=FALSE, cache=TRUE, results='markup'}
...
Area_dNBR <- raster::extract(rdnbr_reclass, AOI_reproj,df = TRUE)
Area_dNBR %>%
  group_by(layer) %>%
  summarize(count = n(), area_meters = n() * (30 * 30))
...
jpeg('output/Prop_RdNBR.jpg',width = 1300, 
     height = 700, res=200, units = "px", quality = 100, pointsize=10)
par(mar=c(4,8,4,4))
pie(table(RdNBR), labels = paste(round(prop.table(table(RdNBR))*100), "%", sep = ""), col = col_val, main = "Proportions of dNBR Classes")
legend('topright',
       legend = legend_val ,fill=col_val,
       inset=c(-0.05,0),
        pt.cex=0.7,cex=0.7,
